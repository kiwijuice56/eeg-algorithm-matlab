% Testing: Plotting the oddball task, raw fNIRS (this is useless)
figure; hold on;

Fs = 256; 
channel = 1;
cutoff = 1000; % Make the plot smaller and easier to handle
...
fnirs_data = read_from_json_file_app(trial, "oddball", "optics");
fnirs_signal = fnirs_data.optics.data(channel, :);
...
% create time vector for fnirs_signal between task start and end
nSamples = numel(fnirs_signal);
...
xlabel('Unix Time (s)');
ylabel('fNIRS amplitude');
title('Oddball fNIRS sample (Fletcher, left ear)');
